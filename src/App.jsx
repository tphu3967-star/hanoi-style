  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        <section className="section product-section" id="products"><div className="section-heading"><div><p className="eyebrow">Món mới mỗi ngày</p><h2>Đang được yêu thích</h2><p className="section-caption">{visibleProducts.length} sản phẩm mẫu · ảnh minh họa từ Unsplash · nguồn marketplace chỉ là demo</p></div><button className="text-link">Xem thêm <span>→</span></button></div>{visibleProducts.length ? <div className="product-grid">{visibleProducts.map((product) => <ProductCard key={product.id} product={product} onSelect={() => setSelectedProduct(product)} />)}</div> : <div className="empty-state"><span>◌</span><h3>Chưa tìm thấy sản phẩm</h3><p>Thử từ khóa, nguồn hoặc danh mục khác nhé.</p></div>}</section>
