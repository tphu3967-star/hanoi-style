...
function App() {
  ...
  }), [activeCategory, query, catalog.shops])
  const visibleProducts = catalog.products.filter((product) => !selectedShop || product.shopId === selectedShop.id)
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        <section className="hero"><div className="hero-copy"><p className="eyebrow"><span className="spark">✦</span> Local fashion, local love</p><h1>Mặc chất<br /><em>Hà Nội.</em></h1><p className="hero-intro">Tìm những cửa hàng thời trang nữ hay ho nhất quanh bạn — từ những con phố thân quen đến góc nhỏ chưa từng biết.</p><div className="search-box"><Icon size={20}>⌕</Icon><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Bạn đang tìm gì hôm nay?" aria-label="Tìm kiếm cửa hàng" />{query && <button className="clear-search" onClick={() => setQuery('')} aria-label="Xóa tìm kiếm">×</button>}<button className="search-submit" onClick={() => scrollTo('shops')}>Tìm kiếm</button></div><div className="quick-search"><span>Tìm kiếm phổ biến</span><button onClick={() => setQuery('váy')}>váy đi tiệc</button><button onClick={() => setQuery('công sở')}>đồ công sở</button><button onClick={() => setQuery('phụ kiện')}>phụ kiện</button></div></div><div className="hero-art"><div className="hero-image-wrap"><img src="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1100&q=85" alt="Thời trang nữ phong cách Hà Nội" /></div><div className="hero-note note-one"><strong>50+</strong><span>shop local<br />đã tuyển chọn</span></div><div className="hero-note note-two"><span className="mini-avatar">♡</span><span>Được yêu thích<br /><strong>bởi 2.000+ nàng</strong></span></div></div></section>
        <section className="section discover-section" id="shops"><div className="section-heading"><div><p className="eyebrow">Chọn gu của bạn</p><h2>Khám phá theo phong cách</h2></div><button className="text-link" onClick={() => { setActiveCategory('Tất cả'); scrollTo('shops') }}>Xem tất cả <span>→</span></button></div><div className="category-row" aria-label="Danh mục">{catalog.categories.map((category) => <button key={category} className={`category-chip ${activeCategory === category ? 'active' : ''}`} onClick={() => setActiveCategory(category)}>{category}</button>)}</div><LocationNotice status={locationStatus} onRequest={requestLocation} /><div className="shop-grid">{filteredShops.map((shop) => <ShopCard key={shop.id} shop={shop} distance={distanceFor(shop)} favorite={favorites.includes(shop.id)} onFavorite={() => toggleFavorite(shop.id)} onSelect={() => setSelectedShop(shop)} onDirections={() => window.open(createDirectionsUrl(shop, userLocation), '_blank', 'noopener,noreferrer')} />)}</div>{!filteredShops.length && <div className="empty-state"><span>◌</span><h3>Chưa tìm thấy shop phù hợp</h3><p>Thử một từ khóa khác hoặc xem tất cả cửa hàng nhé.</p><button onClick={() => { setQuery(''); setActiveCategory('Tất cả') }}>Xem tất cả shop</button></div>}</section>
        <section className="section product-section" id="products"><div className="section-heading"><div><p className="eyebrow">Món mới mỗi ngày</p><h2>Đang được yêu thích</h2></div><button className="text-link">Xem thêm <span>→</span></button></div><div className="product-grid">{visibleProducts.map((product) => <ProductCard key={product.id} product={product} onSelect={() => setSelectedProduct(product)} />)}</div></section>
...
function ShopCard({ shop, distance, favorite, onFavorite, onSelect, onDirections }) {
  return <article className="shop-card"><div className="card-image"><img src={shop.image} alt={shop.name} loading="lazy" /><button className={`favorite ${favorite ? 'is-favorite' : ''}`} onClick={onFavorite} aria-label={favorite ? `Bỏ lưu ${shop.name}` : `Lưu ${shop.name}`}>{favorite ? '♥' : '♡'}</button><span className="distance"><Icon size={14}>⌖</Icon> {distance}</span></div><div className="card-body"><div className="card-title-row"><h3>{shop.name}</h3>{shop.verified && <span className="verified" title="Đã xác minh">✓</span>}</div><p className="muted">{shop.category}</p><p className="address"><Icon size={15}>⌖</Icon> {shop.address}</p><div className="shop-meta"><span><b>★</b> {shop.rating} <small>({shop.reviews})</small></span><span>{shop.priceRange}</span></div><div className="card-actions"><button className="card-link" onClick={onSelect}>Xem shop <span>→</span></button><button className="map-link" onClick={onDirections} aria-label={`Mở chỉ đường đến ${shop.name}`}>⌖ Chỉ đường</button></div></div></article>
}

function ProductCard({ product, onSelect }) {
  return <article className="product-card" onClick={onSelect} tabIndex="0" role="button" onKeyDown={(event) => event.key === 'Enter' && onSelect()}><div className={`product-image ${product.color}`}><img src={product.image} alt={product.name} loading="lazy" />{product.tag && <span className="product-tag">{product.tag}</span>}<button className="product-heart" onClick={(event) => event.stopPropagation()} aria-label={`Lưu ${product.name}`}>♡</button></div><div className="product-info"><h3>{product.name}</h3><p>{product.price} {product.oldPrice && <del>{product.oldPrice}</del>}</p></div></article>
}
