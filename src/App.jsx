  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <nav className="desktop-nav" aria-label="Điều hướng chính"><button onClick={() => scrollTo('products')}>Sản phẩm</button><button onClick={() => scrollTo('shops')}>Shop</button><button onClick={() => scrollTo('about')}>Về HaNoiStyle</button></nav>
...
function ProductCard({ product, sellerName, onSeller, onSelect }) {
  return <article className="product-card" onClick={onSelect} tabIndex="0" role="button" onKeyDown={(event) => event.key === 'Enter' && onSelect()}><div className={`product-image ${product.color}`}><ImageWithFallback src={product.image} alt={product.imageAlt || product.name} loading="lazy" />{product.tag && <span className="product-tag">{product.tag}</span>}<span className="source-badge">{product.sourceLabel || 'Marketplace mẫu'}</span><span className={`stock-badge ${product.stock === 0 ? 'out' : product.stock < 4 ? 'low' : ''}`}>{product.stockStatus}</span><button className="product-heart" onClick={(event) => event.stopPropagation()} aria-label={`Lưu ${product.name}`}>♡</button></div><div className="product-info"><h3>{product.name}</h3>{sellerName && <p className="product-seller">Từ shop <button type="button" onClick={(event) => { event.stopPropagation(); onSeller?.() }} aria-label={`Xem shop ${sellerName}`}>{sellerName}</button></p>}<p>{product.price} {product.oldPrice && <del>{product.oldPrice}</del>}</p><small>{product.variants?.map((variant) => `${variant.name}: ${variant.values.join(', ')}`).join(' · ')}</small></div></article>
}
